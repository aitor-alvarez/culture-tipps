...
@login_required
def change_profile_type(request, email, user_hash):
	check_hash = InstructorRequests.objects.filter(hash=user_hash).exists()
	if check_hash:
		userobject = User.objects.get(email=email)
		profile = Profile.objects.get(user=userobject)
		if profile:
			if profile.type == 'I':
				return HttpResponse('The user has already instructor permission. Nothing needs to be changed')
			else:
				profile.type = 'I'
				profile.save()
				check_hash.delete()
				return HttpResponse('The user with email ' + str(email) + ' has now instructor permissions')
		else:
			return HttpResponse('The email provided does not match our records')
	else:
		return HttpResponse('The hash provided does not match our records.')
